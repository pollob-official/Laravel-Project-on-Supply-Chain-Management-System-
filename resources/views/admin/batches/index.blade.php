...
<div class="container-fluid py-4">
    {{-- ১. স্মার্ট অ্যানালিটিক্স সেকশন (সাপ্লাই চেইন ড্যাশবোর্ড) --}}
    <div class="row mb-4">
        <div class="col-md-3">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="fw-bold mb-0">{{ $batches->where('current_location', 'Processing')->count() }}</h3>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <h4 class="fw-bold mb-0">Recover Batches</h4>
                            </div>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-md-6 d-flex justify-content-md-end gap-2 mt-3 mt-md-0">
                    <form action="{{ URL::current() }}" method="GET" class="d-flex gap-2">
                        <input type="text" name="search" class="form-control form-control-sm shadow-none" placeholder="Search Batch/Product..." value="{{ request('search') }}" style="width: 250px;">
    ...
    {{-- ৩. মেইন ডাটা টেবিল (Supply Chain Flow Design) --}}
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr style="border-left: 5px solid {{ $batch->qc_status == 'approved' ? '#198754' : '#ffc107' }}">
                            {{-- ১. ব্যাচ নং ও সময় --}}
                            <td class="ps-4">
                                ...
                                <div class="small text-muted"><i class="bi bi-clock"></i> {{ $batch->created_at->format('h:i A') }}</div>
                            </td>

                            {{-- ২. বর্তমান স্টেজ (Journey) --}}
                            <td>
                            ...
                            <td>
                                <div class="d-flex flex-column">
                                    <span class="badge {{ $batch->qc_status == 'approved' ? 'bg-success' : 'bg-warning text-dark' }} align-self-start mb-1">
                                    ...
                                    <button class="btn btn-sm btn-link p-0 text-start text-decoration-none" data-bs-toggle="modal" data-bs-target="#qcModal{{$batch->id}}">
                                        <i class="bi bi-patch-check"></i> Verify Production
                                    </button>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="text-center">
                                <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#qrModal{{$batch->id}}">
                                    {!! QrCode::size(45)->generate(url('public/trace/' . $batch->batch_no)) !!}
                                </a>
                            ...
                            <td class="text-end pe-4">
                                <div class="btn-group shadow-sm border rounded bg-white">
                                    <a href="{{ route('public.trace', $batch->batch_no) }}" target="_blank" class="btn btn-white btn-sm text-info border-end" title="Full Traceability">
                                    ...
                                    <form action="{{ route('batches.approve', $batch->id) }}" method="POST">
                                        ...
                                        <div class="modal-body">
                                            <div class="mb-3">
                                                ...
                                                <select name="grade" class="form-select form-select-sm shadow-none" required>
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <textarea name="qc_remarks" class="form-control form-control-sm" rows="2" placeholder="Describe quality..."></textarea>
                                            </div>
                                        </div>
                                ...
                                <div class="modal-content text-center border-0 shadow-lg">
                                    <div class="modal-body p-4" id="printArea{{$batch->id}}">
                                        ...
                                        <div class="d-inline-block border p-2 bg-white mb-3">
                                            {!! QrCode::size(150)->generate(url('public/trace/' . $batch->batch_no)) !!}
                                        </div>
                                        ...
                                        <button class="btn btn-primary w-100 no-print" onclick="printDiv('printArea{{$batch->id}}')">
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-5">
                                ...
                                <h5 class="text-muted">No supply chain data available.</h5>
                                <p class="text-muted small">Start by creating your first production batch.</p>
                                <a href="{{ route('batches.create') }}" class="btn btn-primary btn-sm">Create New Batch</a>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
...
{{-- প্রিন্টিং এবং স্টাইল স্ক্রিপ্ট --}}
<style>
